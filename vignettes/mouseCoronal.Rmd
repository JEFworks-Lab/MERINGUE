---
title: "Identification of Spatial Patterns in 10X Visium Mouse Coronal Data"
author: "Brendan F. Miller"
date: "October 19th, 2020"
output: rmarkdown::pdf_document
vignette: >
  %\VignetteIndexEntry{Identification of Spatial Patterns in 10X Visium Mouse Coronal Data}
...
data(mouseCoronal)
# loads `mouseCoronal <- list(filteredGenes=filteredGenes,
                              # tissueSpotRotation=tissueSpotRotation,
                              # cluster6CorrMtx=cluster6CorrMtx)
# load("mouseCoronal.rda") 
...
spotPos <- read.csv("spatial/tissue_positions_list.csv",
                    header=FALSE)
geneCounts <- readMM("filtered_feature_bc_matrix/matrix.mtx.gz")
barcodes <- read.csv("filtered_feature_bc_matrix/barcodes.tsv.gz",
                     header=FALSE)
features <- read.csv("filtered_feature_bc_matrix/features.tsv.gz",
                     sep="\t",
                     header=FALSE)
...
# Get overdispersed genes whose expression variance is higher than 
# transcriptome-wide expectations
par(mfrow=c(1,2), mar=c(1,1,1,1))
overdispersedGenes <- getOverdispersedGenes(filteredGenes,
                                            plot = TRUE,
                                            details = TRUE)
...
#filteredGenes <- filteredGenes[order(rowVars(as.matrix(filteredGenes)),
...
# drop `7: Hippocampus` b/c no sig genes found
diffgexp <- signDiffGenes[ names(signDiffGenes)[-7] ]
