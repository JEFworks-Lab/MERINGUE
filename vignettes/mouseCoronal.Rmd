...
# k <- 100 # use to adjust number of clusters
# nn <- nn2(as.matrix(pcs), k = k) # nearest neighbor search
...
# lc.norm <- cluster_louvain(nw.norm) # igraph method for finding communities 
# com <- as.factor(membership(lc.norm)) # get communities of spots
# names(com) <- rownames(pcs)
# 
# spotClusters <- as.character(com)
# names(spotClusters) <- names(com)
...
for (gene in rownames(topDiffExpGenesClust4[1:20,])) {
  ...
  print(gene)
  #plot(c(0,dim(tiff)[2]), c(0,dim(tiff)[1]), main=gene) 
  
  # offTissueCounts.GeneOfInterest <- allgeneCountsFiltCPM[gene, nontissueSpots]
  geneOfInterestExp <- filteredGenes[gene,]
  
  # ramp color for continuous gene expression values
  #coolToHot <- colorRampPalette(c('blue','red'))
  #colors <- coolToHot(10)[as.numeric(cut(log10(geneOfInterestExp + 1), breaks = 10))]
  
  # points(spotPosTiff[,]$V6, spotPosTiff[,]$V5, col=colors, pch=15) # all spots
  # points(spotPosTiff[rownames(tissueSpotPosTiff),]$V6, spotPosTiff[rownames(tissueSpotPosTiff),]$V5, col="white", pch="+") # tissue spots
  
  dfexp <- data.frame(
                 width_X = tissueSpotRotation$V6,
                 height_Y = tissueSpotRotation$V5,
                 exp = geneOfInterestExp)
                 #exp = log10(geneOfInterestExp + 1))
  
  
  print(ggplot(dfexp, aes(width_X, height_Y)) +                     
    geom_point(aes(color = exp), size = 1, shape=15) +
...
```{r visualize-top-pca1-driving-genes, fig.width=5, fig.height=5, eval=FALSE}
...
```{r visualize-top-pca5-driving-genes, fig.width=5, fig.height=5, eval=FALSE}
...
```{r visualize-clust4-pattern1-genes, fig.width=5, fig.height=5, eval=FALSE, include=FALSE}
...
```{r visualize-clust4-pattern2-genes, fig.width=5, fig.height=5, eval=FALSE, include=FALSE}
...
```{r visualize-clust4-pattern3-genes, fig.width=5, fig.height=5, eval=FALSE, include=FALSE}
...
```{r visualize-clust4-pattern4-genes, fig.width=5, fig.height=5, eval=FALSE, include=FALSE}
...
```{r visualize-top-diff-exp-genes-cluster6, fig.width=5, fig.height=5, eval=TRUE}
