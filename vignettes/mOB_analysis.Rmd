...
dg <- do.call(rbind, lapply(test, function(g) {
  do.call(cbind, lapply(levels(annot), function(ct) {
    x <- mat[g, names(annot)[annot == ct]]
    y <- mat[g, names(annot)[annot != ct]]
    wilcox.test(x, y, alternative='greater')$p.val
  }))
}))
colnames(dg) <- levels(annot)
rownames(dg) <- test

# Bonferonni correct p-value < 0.05
dg.sig <- apply(dg, 2, function(x) {
  x <- p.adjust(x, n=length(dg))
  names(x)[x < 0.05]
})
...
w <- getSpatialWeights(pos, klist=9)
...
Spatial analysis can also be complementary to spatially-unaware clustering analysis. For example, after identifying putative cell-types by spatially-unaware clustering analysis, we may be interesting in identifying genes that exhibit spatial clustering within this cell-type. For demonstration purposes, we will focus on testing whether any marker genes for the Olfactory Nerve Layer exhibit spatial clustering within the Olfactory Nerve Layer. 

```{r, spatial-sub-network, fig.width=4, fig.height=3}
# Restrict to just voxels corresponding to the Olfactory Nerve Layer
sub <- names(annot)[annot == '5: Olfactory Nerve Layer']
...
# Limit to marker genes for the Olfactory Nerve Layer
...
scc.sub <- spatialCrossCorMatrix(sigMat = as.matrix(mat[results.filter.sub, sub]), 
                                 w = w.sub)
ggroup.sub <- groupSigSpatialPatterns(pos = pos, 
                                      mat = as.matrix(mat[results.filter.sub, sub]), 
                                      scc = scc.sub, 
                                      power = 1, 
                                      hclustMethod = 'ward.D', 
                                      deepSplit = 4)
...
```{r, spatial-sub-heatmap, fig.width=4, fig.height=4}
# Visualize as heatmaps
gcol <- rainbow(length(levels(ggroup.sub$groups)), v=0.5)[ggroup.sub$groups]
  names(gcol) <- names(ggroup.sub$groups)
heatmap(scc.sub[ggroup.sub$hc$labels, ggroup.sub$hc$labels], scale='none', 
        Colv=as.dendrogram(ggroup.sub$hc), 
        Rowv=as.dendrogram(ggroup.sub$hc), 
        labRow=NA, labCol=NA,
        ColSideColors=gcol[ggroup.sub$hc$labels],
        RowSideColors=gcol[ggroup.sub$hc$labels],
        col=colorRampPalette(c('blue', 'white', 'red'))(100)
)
...
m[m < -2.5] <- -2.5
m[m > 2.5] <- 2.5
heatmap(m, scale='none', Rowv=NA, labRow=NA, labCol=NA,
          RowSideColors=gcol[rownames(m)],
          col=colorRampPalette(c('blue', 'white', 'red'))(100)
)
```
