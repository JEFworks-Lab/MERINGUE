---
title: "Multi-Layer Breast Cancer Spatial Transcriptomics Analysis with MERINGUE"
author: "Jean Fan"
date: "`r Sys.Date()`"
...
data(BCL) # object contains counts and positions for all 4 layers. Will only need layers 1-3

BCL1.pos <- BCL$pos[which(grepl("L1-", rownames(BCL$pos))),][,c("x", "y")]
BCL2.pos <- BCL$pos[which(grepl("L2-", rownames(BCL$pos))),][,c("x", "y")]
BCL3.pos <- BCL$pos[which(grepl("L3-", rownames(BCL$pos))),][,c("x", "y")]

BCL1.counts <- BCL$counts[,which(grepl("L1-", colnames(BCL$counts)))]
BCL2.counts <- BCL$counts[,which(grepl("L2-", colnames(BCL$counts)))]
BCL3.counts <- BCL$counts[,which(grepl("L3-", colnames(BCL$counts)))]
...
```{r, bcl-combine}
# Get common set of genes for the layers
...
# Combine into large counts matrix. Counts of genes for all the layers being assessed
counts <- cbind(
...
gdiff <- gdiff[order(L3$I[gdiff,]$observed, decreasing=TRUE)]
